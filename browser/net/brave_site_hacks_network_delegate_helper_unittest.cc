...
TEST(BraveSiteHacksNetworkDelegateHelperTest, QueryStringUntouched) {
  const std::vector<const std::string> urls(
      {"https://example.com/", "https://example.com/?",
       ...
       "https://example.com/?+fbclid=1", "https://example.com/?%20fbclid=1",
...
TEST(BraveSiteHacksNetworkDelegateHelperTest, QueryStringFiltered) {
  const std::vector<const std::pair<const std::string, const std::string>> urls(
      {// { original url, expected url after filtering }
       ...
       {"https://example.com/?mkt_unsubscribe=0&fake_param=abc&mkt_tok=123",
        "https://example.com/?mkt_unsubscribe=0&fake_param=abc"},
       {"https://example.com/"
        "?test_mkt_unsubscribe=1&fake_param=abc&mkt_tok=123",
        "https://example.com/?test_mkt_unsubscribe=1&fake_param=abc"}});
