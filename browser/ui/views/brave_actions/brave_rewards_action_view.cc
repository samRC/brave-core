...
BraveRewardsActionView::BraveRewardsActionView(Browser* browser)
  ...
  RewardsPanelCoordinator::RegisterForBrowser(browser_);
  if (auto* coordinator = RewardsPanelCoordinator::FromBrowser(browser_)) {
    panel_observation_.Observe(coordinator);
  }
...
void BraveRewardsActionView::OnButtonPressed() {
  ...
  // background for cached access and handling panel requests. In order to
  // notify the panel, request that the panel be opened through the panel
  // coordinator, rather than opening the bubble directly.
  if (auto* coordinator = RewardsPanelCoordinator::FromBrowser(browser_)) {
