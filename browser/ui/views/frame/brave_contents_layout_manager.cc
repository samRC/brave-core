...
void BraveContentsLayoutManager::Layout(views::View* host) {
  ...
  const int vertical_tab_width =
      vertical_tabs_container_->GetPreferredSize().width();
  gfx::Rect vertical_tab_bounds(sidebar_width, 0, vertical_tab_width, height);
  gfx::Rect contents_bounds(vertical_tab_bounds.right(), 0,
                            width - sidebar_width - vertical_tab_width, height);
