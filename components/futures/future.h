...
class FutureState : public base::RefCountedThreadSafe<FutureState<T>> {
 ...
 public:
  ...
  void SetValue(T value) {
    if (task_runner_->RunsTasksInCurrentSequence()) {
      SetValueInTask(std::move(value), false);
    } else {
      task_runner_->PostTask(FROM_HERE,
                             base::BindOnce(&FutureState::SetValueInTask, this,
                                            std::move(value), true));
    }
  }
  ...
  void SetListener(base::OnceCallback<void(T)> on_complete) {
    ...
    if (task_runner_->RunsTasksInCurrentSequence()) {
      SetListenerInTask(std::move(listener));
    } else {
      task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&FutureState::SetListenerInTask, this,
                                    std::move(listener)));
    }
  }
 ...
 private:
  ...
  void SetValueInTask(T value, bool in_task) {
    ...
    if (listener_) {
      ...
      if (in_task && listener.task_runner->RunsTasksInCurrentSequence()) {
        // If we are setting the value in a posted task and we're currently on
        // the listener's task runner, then there is no need to post another
        // task to execute the callback. We can just execute it directly.
        RunCompleteCallback(std::move(value), std::move(listener.on_complete));
      } else {
        listener.task_runner->PostTask(
            FROM_HERE, base::BindOnce(RunCompleteCallback, std::move(*value_),
                                      std::move(listener.on_complete)));
